...
class Chatter:
    def __init__(self,
        ...
        self.adapter_path = adpater_path
        self.model, self.tokenizer = self.load_model(efficient_load)
        self.template = json_load(template_path)


    def load_model(self, efficient_load=False):
    ...
    def _init_generate_kwargs(self, src_tok, attention_mask, is_greedy=False):
        if is_greedy:
            return {
                'input_ids': src_tok,
    ...
    async def generate(self, websocket):
        async for raw_message in websocket.iter_text():
            ...
            src_tok, user_prompt = self.preprocess(user_prompt, system_prompt)
            ...
            response = []
            generate_kwargs = self._init_generate_kwargs(src_tok, attention_mask, self.is_greedy)
